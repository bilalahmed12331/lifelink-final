...
const getDashboard = async (req, res) => {
    try {
        const [users] = await pool.query(
            'SELECT id, full_name, blood_group, availability, last_donation FROM users WHERE id = ?',
            [req.user.id]
        );
        ...
        const [history] = await pool.query(
            'SELECT id, donation_date, location, created_at FROM donation_history WHERE user_id = ? ORDER BY donation_date DESC',
            [req.user.id]
        );
...
const updateDonationInfo = async (req, res) => {
    try {
        ...
        const [currentUser] = await pool.query(
            'SELECT last_donation FROM users WHERE id = ?',
            [req.user.id]
        );
        ...
        const currentLastDonation = currentUser[0].last_donation;
        
        await pool.query(
            `UPDATE users SET blood_group = ?, city = ?, phone = ?, last_donation = ?, availability = ?, medical_notes = ? 
             WHERE id = ?`,
        ...
        if (lastDonation && lastDonation !== currentLastDonation) {
            await pool.query(
                'INSERT INTO donation_history (user_id, donation_date, location) VALUES (?, ?, ?)',
                [req.user.id, lastDonation, city || '']
            );
        }
        
        const [updatedUser] = await pool.query(
            'SELECT id, full_name, blood_group, city, phone, availability, last_donation, medical_notes FROM users WHERE id = ?',
            [req.user.id]
        );
...
const updateAvailabilityOnly = async (req, res) => {
    try {
        ...
        const currentAvailability = currentUser[0].availability;
